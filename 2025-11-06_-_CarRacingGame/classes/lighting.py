...
class StreetLight:
    ...
    def __init__(self, x, y, z, light_id):
        ...
        # Collision bounds (for pole at base)
        self.collision_radius = 0.15  # Radius of pole collision
        self.collision_height = self.pole_height  # Height of collision from ground
    ...
    def check_collision(self, car_x, car_z, car_size=0.6):
        """Check if the car collides with this street light pole"""
        # Calculate distance from car center to light pole center
        dx = car_x - self.x
        dz = car_z - self.z
        distance = (dx**2 + dz**2) ** 0.5
        
        # Collision if distance is less than sum of radii
        return distance < (car_size + self.collision_radius)
...
class TrafficLight:
    ...
    def __init__(self, x, y, z, direction, light_id, rotation=0):
        ...
        # Collision bounds (for pole at base)
        self.collision_radius = 0.2  # Slightly larger than street light pole
        self.collision_height = self.pole_height
    ...
    def check_collision(self, car_x, car_z, car_size=0.6):
        """Check if the car collides with this traffic light pole"""
        # Calculate distance from car center to light pole center
        dx = car_x - self.x
        dz = car_z - self.z
        distance = (dx**2 + dz**2) ** 0.5
    ...
    def setup_global_lighting(self):
        """Set up global lighting properties for the scene."""
        glEnable(GL_LIGHTING)
        glEnable(GL_LIGHT0)
