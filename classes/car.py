...
class Vehicle(Physics):
    ...
    def __init__(self, vehicle_type="car"):
        ...
        self.vertical_velocity = 0.0  # Velocity in Y direction
        self.gravity = 0.01  # Gravity acceleration (9.81 m/s^2 equivalent scaled)
        self.weight = 1.0  # Car weight factor
        
        # Steering wheel angle for front wheel rotation
        self.steering_angle = 0.0
        
        # Wheel rotation for visual effect
        self.wheel_rotation = 0.0  # Cumulative wheel rotation angle
        ...
        self.on_sidewalk = False  # Track if car is on a sidewalk
...
class Car(Vehicle):
    ...
    def update(self, keys, is_day_time=True):
        ...
        if keys[pygame.K_l]:
            ...
            if current_time - self.last_headlight_toggle > 300:  # 300ms delay
                ...
                print_terminal("INFO", "Headlights", f"{'ON' if self.headlights_on else 'OFF'}")
        ...
        if self.auto_headlights:
            if not is_day_time and not self.headlights_on:
                ...
                print_terminal("INFO", "Headlights", "Auto headlights ON (nighttime)")
            elif is_day_time and self.headlights_on and not hasattr(self, 'manual_headlights'):
                self.headlights_on = False
                print_terminal("INFO", "Headlights", "Auto headlights OFF (daytime)")
        ...
        if keys[pygame.K_t]:
            ...
            if current_time - self.last_transmission_toggle > 500:  # 500ms delay
                ...
                print_terminal("INFO", "Transmission", f"{'Auto' if self.auto_transmission else 'Manual'}")
        ...
        wheel_circumference = 2 * math.pi * 0.15  # radius = 0.15
        self.wheel_rotation += (self.speed / wheel_circumference) * 360.0  # Convert to degrees
        if self.wheel_rotation > 360.0:
            self.wheel_rotation -= 360.0
        elif self.wheel_rotation < -360.0:
            self.wheel_rotation += 360.0
        ...
        # Note: The draw method adds 0.30 to Y, so ground is at Y=0.0
        ground_height = 0.0  # Ground level (draw adds 0.30 to render at 0.30)
        
        # Check if car is above ground
        if self.y > ground_height:
            # Apply gravity - car falls
            self.vertical_velocity -= self.gravity * self.weight
    ...
    def resolve_collision(self, prev_x, prev_z):
        ...
        self.z = prev_z
        self.speed = 0.0
        # Ensure car is on ground when collision happens
        self.y = 0.0
        self.vertical_velocity = 0.0
    
    def check_barrier_collision(self):
        """Check if the car collides with map barriers at the complete perimeter edges"""
        car_size = 0.6
        barrier_thickness = 0.3
        
        # Map edges where barriers exist (complete perimeter at ±150)
        ...
        # Check NORTH barrier (complete edge at Z = +150, X from -150 to +150)
        if (car_back > map_edge_north - barrier_thickness and 
            car_front < map_edge_north and
            car_right > map_edge_west and car_left < map_edge_east):
            return True
        
        # Check SOUTH barrier (complete edge at Z = -150, X from -150 to +150)
        if (car_front < map_edge_south + barrier_thickness and 
            car_back > map_edge_south and
            car_right > map_edge_west and car_left < map_edge_east):
            return True
        
        # Check EAST barrier (complete edge at X = +150, Z from -150 to +150)
        if (car_right > map_edge_east - barrier_thickness and 
            car_left < map_edge_east and
            car_back > map_edge_south and car_front < map_edge_north):
            return True
        
        # Check WEST barrier (complete edge at X = -150, Z from -150 to +150)
        if (car_left < map_edge_west + barrier_thickness and 
            car_right > map_edge_west and
            car_back > map_edge_south and car_front < map_edge_north):
            return True
    ...
    def draw(self):
        ...
        glPushMatrix()
        glTranslatef(self.x, self.y + 0.30, self.z)  # Raise car above track (0.30 for larger wheels with radius 0.15)
        ...
        # Ensure wheels have correct dark gray color and lighting is enabled
        glEnable(GL_LIGHTING)
        glColor3f(0.05, 0.05, 0.05)  # Very dark gray wheel color

        # Front left wheel (adjusted for narrower car)
        glPushMatrix()
        glDisable(GL_LIGHTING)  # Disable lighting for consistent wheel color
        glColor3f(0.05, 0.05, 0.05)  # Very dark gray wheels
        ...
        glRotatef(self.wheel_rotation, 1, 0, 0)  # Rotate around X-axis based on speed
        self.draw_wheel()
        glEnable(GL_LIGHTING)  # Re-enable lighting
        glPopMatrix()
    ...
    def draw_wheel(self):
        """Draw a wheel as a cylinder with rotation"""
        radius = 0.15
        height = 0.2
        segments = 16
        
        glPushMatrix()
        # Rotate cylinder 90 degrees so it's aligned for Z-axis rotation
        glRotatef(90, 0, 0, 1)
    ...
    def is_on_sidewalk(self, tracks):
        """Check if car is currently on a sidewalk surface"""
        car_size = 0.6
        car_left = self.x - car_size
        car_right = self.x + car_size
        car_front = self.z - car_size
        ...
        for track in tracks:
            pavement_width = track.pavement_width  # 2.0
            track_width = track.track_width  # 4.0
            arm_length = track.arm_length  # 30.0
            
            # Calculate sidewalk boundaries
            pavement_inner_north = track_width/2 + track.pos_y
            pavement_outer_north = track_width/2 + pavement_width + track.pos_y
            pavement_inner_south = -track_width/2 + track.pos_y
            pavement_outer_south = -track_width/2 - pavement_width + track.pos_y
            pavement_inner_east = track_width/2 + track.pos_x
            pavement_outer_east = track_width/2 + pavement_width + track.pos_x
            pavement_inner_west = -track_width/2 + track.pos_x
            pavement_outer_west = -track_width/2 - pavement_width + track.pos_x
            
            horizontal_gap = 2.0
            vertical_gap = 4.0
            
            # Check all 8 sidewalk sections
            # North side pavement - west section
            if (car_left > -arm_length + track.pos_x and car_right < -horizontal_gap + track.pos_x and
                car_front < pavement_outer_north and car_back > pavement_inner_north):
                return True
            # North side pavement - east section
            if (car_left > horizontal_gap + track.pos_x and car_right < arm_length + track.pos_x and
                car_front < pavement_outer_north and car_back > pavement_inner_north):
                return True
            # South side pavement - west section
            if (car_left > -arm_length + track.pos_x and car_right < -horizontal_gap + track.pos_x and
                car_front < pavement_inner_south and car_back > pavement_outer_south):
                return True
            # South side pavement - east section
            if (car_left > horizontal_gap + track.pos_x and car_right < arm_length + track.pos_x and
                car_front < pavement_inner_south and car_back > pavement_outer_south):
                return True
            # East side pavement - north section
            if (car_left > pavement_inner_east and car_right < pavement_outer_east and
                car_front < arm_length + track.pos_y and car_back > vertical_gap + track.pos_y):
                return True
            # East side pavement - south section
            if (car_left > pavement_inner_east and car_right < pavement_outer_east and
                car_front < -vertical_gap + track.pos_y and car_back > -arm_length + track.pos_y):
                return True
            # West side pavement - north section
            if (car_left > pavement_outer_west and car_right < pavement_inner_west and
                car_front < arm_length + track.pos_y and car_back > vertical_gap + track.pos_y):
                return True
            # West side pavement - south section
            if (car_left > pavement_outer_west and car_right < pavement_inner_west and
                car_front < -vertical_gap + track.pos_y and car_back > -arm_length + track.pos_y):
                return True
    ...
    def check_sidewalk_collision(self, tracks):
        """Check if car is colliding with sidewalk edge (curb) - blocks entry from road"""
        car_size = 0.5  # Reduced from 0.6 to allow easier climbing
        car_left = self.x - car_size
        car_right = self.x + car_size
        car_front = self.z - car_size
        car_back = self.z + car_size
        ...
        for track in tracks:
            ...
            horizontal_gap = 2.0
            vertical_gap = 4.0
            curb_thickness = 0.4  # Tolerance for curb collision
            
            # Check curb/edge collisions - these create a barrier to entering the sidewalk
            # BUT exclude the crossing gaps where curbs are cut
            
            # North sidewalk edges (inner edge of north pavement) - west of crossing
            if car_back > pavement_inner_north - curb_thickness and car_back < pavement_inner_north + curb_thickness:
                if (car_left > -arm_length + track.pos_x and car_right < -horizontal_gap + track.pos_x):
                    return True
            # North sidewalk edges - east of crossing
            if car_back > pavement_inner_north - curb_thickness and car_back < pavement_inner_north + curb_thickness:
                if (car_left > horizontal_gap + track.pos_x and car_right < arm_length + track.pos_x):
                    return True
            
            # South sidewalk edges (inner edge of south pavement) - west of crossing
            if car_front > pavement_inner_south - curb_thickness and car_front < pavement_inner_south + curb_thickness:
                if (car_left > -arm_length + track.pos_x and car_right < -horizontal_gap + track.pos_x):
                    return True
            # South sidewalk edges - east of crossing
            if car_front > pavement_inner_south - curb_thickness and car_front < pavement_inner_south + curb_thickness:
                if (car_left > horizontal_gap + track.pos_x and car_right < arm_length + track.pos_x):
                    return True
            
            # East sidewalk edges (inner edge of east pavement) - north of crossing
            if car_right > pavement_inner_east - curb_thickness and car_right < pavement_inner_east + curb_thickness:
                if (car_front > vertical_gap + track.pos_y and car_back < arm_length + track.pos_y):
                    return True
            # East sidewalk edges - south of crossing
            if car_right > pavement_inner_east - curb_thickness and car_right < pavement_inner_east + curb_thickness:
                if (car_front > -arm_length + track.pos_y and car_back < -vertical_gap + track.pos_y):
                    return True
            
            # West sidewalk edges (inner edge of west pavement) - north of crossing
            if car_left > pavement_inner_west - curb_thickness and car_left < pavement_inner_west + curb_thickness:
                if (car_front > vertical_gap + track.pos_y and car_back < arm_length + track.pos_y):
                    return True
            # West sidewalk edges - south of crossing
            if car_left > pavement_inner_west - curb_thickness and car_left < pavement_inner_west + curb_thickness:
                if (car_front > -arm_length + track.pos_y and car_back < -vertical_gap + track.pos_y):
                    return True
...
class Motorcycle(Vehicle):
    ...
    def update(self, keys, is_day_time=True):
        ...
        if keys[pygame.K_l]:
            ...
            if current_time - self.last_headlight_toggle > 300:
                self.headlights_on = not self.headlights_on
                self.last_headlight_toggle = current_time
                print_terminal("INFO", "Headlight", f"Motorcycle headlight: {'ON' if self.headlights_on else 'OFF'}")
        
        # Auto headlights based on time of day
        if self.auto_headlights:
            if not is_day_time and not self.headlights_on:
                self.headlights_on = True
                print_terminal("INFO", "Headlight", "Auto motorcycle headlight ON (nighttime)")
            elif is_day_time and self.headlights_on:
                self.headlights_on = False
                print_terminal("INFO", "Headlight", "Auto motorcycle headlight OFF (daytime)")
    ...
    def resolve_collision(self, prev_x, prev_z):
        ...
        self.z = prev_z
        self.speed = 0.0
        # Ensure motorcycle is on ground when collision happens
        self.y = 0.0
        self.vertical_velocity = 0.0
    
    def check_barrier_collision(self):
        """
        Check if the motorcycle collides with any barriers on the track.
        This function should handle collision detection and response for barriers.
        """
        # Placeholder logic for barrier collision detection
        # Replace with actual game logic as needed
        for barrier in self.track_barriers:
            if self.x < barrier.x + barrier.width and self.x + self.width > barrier.x and \
               self.z < barrier.z + barrier.length and self.z + self.length > barrier.z:
                # Collision detected, handle response
                self.resolve_collision(self.x, self.z)
                break
    ...
    def draw(self):
        ...
        glPushMatrix()
        glTranslatef(self.x, self.y + 0.30, self.z)  # Raise motorcycle above track (0.30 for larger wheels)
        glRotatef(self.angle, 0, 1, 0)
    ...
    def draw_wheel(self):
        ...
        size = 0.15  # Increased from 0.08 for larger wheels that can climb curbs
        # Front face
...
class NPCCar(Car):
    ...
    def __init__(self, x, z, direction, speed=0.3):
        ...
        self.stop_distance = 6.0  # Distance to stop before traffic light (reduced for new car size)
    ...
    def check_car_collision(self, other_cars):
        ...
        collision_distance = 4.0  # Minimum safe distance between cars (adjusted for new car size)
    ...
    def draw_headlight_glow(self):
        """
        Draw a transparent glow effect for the car's headlights at night.
        """
        if not self.headlights_on:
            return  # No glow if headlights are off

        glEnable(GL_BLEND)
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        glColor4f(1.0, 1.0, 0.8, 0.3)  # Soft yellowish glow with transparency

        # Define the glow area in front of the car
        glow_radius = 1.5
        glow_length = 3.0
        glBegin(GL_QUADS)
        glVertex3f(self.x - glow_radius, self.y, self.z + glow_length)
        glVertex3f(self.x + glow_radius, self.y, self.z + glow_length)
        glVertex3f(self.x + glow_radius, self.y, self.z)
        glVertex3f(self.x - glow_radius, self.y, self.z)
        glEnd()
    ...
    def draw(self):
        ...
        # Add tail lights (automatically on at night)
        if not hasattr(self, '_last_is_day_time'):
            self._last_is_day_time = True
