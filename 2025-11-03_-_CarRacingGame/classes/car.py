...
class Car(Vehicle):
    def draw_chassis(self):
        ...
        # Outer chassis (as before)
        glColor3f(0.08, 0.08, 0.08)  # Very dark gray, almost black
        glBegin(GL_QUADS)
        y_bottom = -0.1
        y_top = 0.18
        x_left = -0.5
        ...
        # Add roof and pillars between windows
        glColor3f(0.08, 0.08, 0.08)  # VW blue color for roof
        glBegin(GL_QUADS)
        ...
        # Left A-pillar
        glVertex3f(-0.4, 0.25, 0.4)    # Windshield base left
        glVertex3f(-0.35, 0.58, 0.1)   # Windshield top left
        glVertex3f(-0.35, 0.58, 0.1)   # Side window top front (same point)
        glVertex3f(-0.5, 0.25, 0.4)    # Side window base front
        
        # Right A-pillar
        glVertex3f(0.4, 0.25, 0.4)     # Windshield base right
        glVertex3f(0.5, 0.25, 0.4)     # Side window base front
        glVertex3f(0.35, 0.58, 0.1)    # Side window top front (same as windshield)
        glVertex3f(0.35, 0.58, 0.1)    # Windshield top right
        ...
        glVertex3f(-0.35, 0.58, -0.9)  # Rear window top left
        glVertex3f(-0.35, 0.58, -0.9)  # Side window rear top (same point)
        
        # Right C-pillar
        glVertex3f(0.5, 0.25, -0.7)    # Side window rear base
        glVertex3f(0.35, 0.58, -0.9)   # Side window rear top (same as rear window)
        glVertex3f(0.35, 0.58, -0.9)   # Rear window top right
        ...
        # # fill the gaps between the windows with an inner chassis, but you can see through the windows
        # glBegin(GL_QUADS)
        # glColor4f(0.1, 0.1, 0.1, 0.3)  # Very dark gray with some transparency
        # # Left side inner chassis between windows
        # glVertex3f(-0.4, 0.262, 0.52)
        # glVertex3f(-0.488, 0.262, 0.888)
        # glVertex3f(-0.488, 0.262, -0.6)
        # glVertex3f(-0.35, 0.592, -0.112)
        # # Right side inner chassis between windows
        # glVertex3f(0.5, 0.05, 0.4)
        # glVertex3f(0.5, 0.25, 0.4)
        # glVertex3f(0.5, 0.25, -0.7)
        # glVertex3f(0.5, 0.05, -0.7)
        # glEnd()

        # # Inner chassis - visible through windows
        # glBegin(GL_QUADS)
        # glColor4f(0.1, 0.1, 0.1, 0.3)  # Very dark gray with some transparency
        # # Front inner chassis below windshield
        # glVertex3f(-0.4, 0.05, 0.4)
        # glVertex3f(0.4, 0.05, 0.4)
        # glVertex3f(0.4, 0.25, 0.4)
        # glVertex3f(-0.4, 0.25, 0.4)
        # # Rear inner chassis below rear window
        # glVertex3f(-0.4, 0.05, -1.2)
        # glVertex3f(0.4, 0.05, -1.2)
        # glVertex3f(0.4, 0.25, -1.2)
        # glVertex3f(-0.4, 0.25, -1.2)
        # glEnd()
    ...
    def __init__(self):
        super().__init__("car")
        # Remove duplicate properties that are now in Vehicle base class
        self.max_speed = 0.8  # Increased max speed for 6 gears
        self.turn_speed = 6.0  # Increased from 3.0 to 6.0 for sharper turning
        ...
        self.max_rpm = 6000.0
        self.idle_rpm = 800.0
        self.shift_up_rpm = 5500.0
        self.shift_down_rpm = 2000.0
        self.auto_transmission = True  # Can be toggled
        
        # Lighting system - moved to Vehicle base class
        
        # Gear ratios (higher ratios for lower gears = more acceleration, less top speed)
        self.gear_ratios = {
            1: 3.5,   # 1st gear - high acceleration, low speed
            2: 2.2,   # 2nd gear
            3: 1.6,   # 3rd gear
            4: 1.2,   # 4th gear
            5: 0.9,   # 5th gear
            6: 0.7    # 6th gear - low acceleration, high speed
        }
        
        # Speed limits for each gear (as fraction of max_speed)
        self.gear_speed_limits = {
            1: 0.15,  # 1st gear max speed
            2: 0.25,  # 2nd gear max speed
            3: 0.40,  # 3rd gear max speed
            4: 0.60,  # 4th gear max speed
            5: 0.80,  # 5th gear max speed
            6: 1.0    # 6th gear max speed (full speed)
        }
    ...
    def update(self, keys, is_day_time=True):
        ...
        if keys[pygame.K_DOWN]:
            self.braking = True
            self.speed = max(self.speed - self.base_acceleration * 2, -self.max_speed/3)
        ...
        # Update position
        new_x = self.x + math.sin(math.radians(self.angle)) * self.speed
        new_z = self.z + math.cos(math.radians(self.angle)) * self.speed
    ...
    def auto_shift(self):
        ...
        # Shift up if RPM is too high and not in top gear
        if self.rpm >= self.shift_up_rpm and self.current_gear < 6:
            self.shift_up()
    ...
    def draw(self):
        ...
        # Car body (VW Golf style hatchback)
        glColor3f(0.05, 0.15, 0.7)  # Deep blue typical for VW
        glBegin(GL_QUADS)
        glEnd()
        ...
        # Add brake lights (red lights at back when braking) - adjusted for larger car
        if self.braking:
            glDisable(GL_LIGHTING)
        ...
        if self.turning_left:
            ...
            if blink:
                ...
                self.draw_wheel()
                glPopMatrix()
                # Rear right turn signal - BIGGER (adjusted for narrower car)
                glPushMatrix()
                glTranslatef(0.55, 0.2, -1.0)  # Adjusted position for narrower car
                glScalef(0.24, 0.24, 0.24)  # Increased from 0.05 to 0.12
                self.draw_wheel()
                ...
                self.draw_wheel()
                glPopMatrix()
                # Additional glow effect for rear right (adjusted for narrower car)
                glPushMatrix()
                glTranslatef(0.55, 0.2, -1.1)  # Adjusted position
                glScalef(0.15, 0.15, 0.08)  # Even bigger glow
                self.draw_wheel()
        ...
        if self.turning_right:
            ...
            if blink:
                ...
                self.draw_wheel()
                glPopMatrix()
                # Rear left turn signal - BIGGER (adjusted for narrower car)
                glPushMatrix()
                glTranslatef(-0.55, 0.2, -1.0)  # Adjusted position for narrower car
                glScalef(0.12, 0.12, 0.12)  # Increased from 0.05 to 0.12
                self.draw_wheel()
        ...
        glColor3f(0.2, 0.2, 0.2)  # Reset to wheel color
    ...
    def draw_windows(self):
        ...
        # glColor4f(0.1, 0.3, 0.5, 0.6)
        # glBegin(GL_QUADS)
        # glVertex3f(-0.5 - offset, 0.25 + offset, 0.8)    # Front bottom
        # glVertex3f(-0.5 - offset, 0.25 + offset, 0.2)    # Rear bottom
        # glVertex3f(-0.46 - offset, 0.58 + offset, 0.15)  # Rear top
        # glVertex3f(-0.46 - offset, 0.58 + offset, 0.75)  # Front top
        # glEnd()

        # # Left rear door window
        # glColor4f(0.1, 0.3, 0.5, 0.6)
        # glBegin(GL_QUADS)
        # glVertex3f(-0.5 - offset, 0.25 + offset, 0.1)    # Front bottom
        # glVertex3f(-0.5 - offset, 0.25 + offset, -0.5)   # Rear bottom
        # glVertex3f(-0.46 - offset, 0.58 + offset, -0.45) # Rear top
        # glVertex3f(-0.46 - offset, 0.58 + offset, 0.05)  # Front top
        # glEnd()
        ...
        # # Enhanced rear window
        # glColor4f(0.1, 0.3, 0.5, 0.6)
        # glBegin(GL_QUADS)
        # glVertex3f(-0.38, 0.58 + offset, -0.85 - offset)  # Top left
        # glVertex3f(0.38, 0.58 + offset, -0.85 - offset)   # Top right
        # glVertex3f(0.4, 0.25 + offset, -1.2 - offset)     # Bottom right
        # glVertex3f(-0.4, 0.25 + offset, -1.2 - offset)    # Bottom left
        # glEnd()
        
        # (All gloss stripes, white overlays, and reflection polygons removed)
...
class Motorcycle(Vehicle):
    ...
    def __init__(self):
        ...
        self.max_speed = 1.0  # Faster than car
        self.base_acceleration = 0.015  # Faster acceleration
        self.turn_speed = 8.0  # More agile turning
        ...
        # Gear ratios for motorcycle (more aggressive)
        self.gear_ratios = {
            1: 4.0,   # 1st gear - very high acceleration
            2: 2.5,   # 2nd gear
            3: 1.8,   # 3rd gear
            4: 1.3,   # 4th gear
            5: 1.0    # 5th gear - high speed
        }
    ...
    def update(self, keys, is_day_time=True):
        ...
        if keys[pygame.K_DOWN]:
            self.braking = True
            self.speed = max(self.speed - self.base_acceleration * 2.5, -self.max_speed/4)  # Faster braking
    ...
    def draw(self):
        ...
        # Add brake light (single centered red light at back when braking)
        if self.braking:
            glDisable(GL_LIGHTING)
...
class NPCCar(Car):
    ...
    def __init__(self, x, z, direction, speed=0.3):
        ...
        self.stop_distance = 3.0  # Distance to stop before traffic light
    ...
    def update_ai(self, traffic_lights):
        """Update NPC car AI behavior"""
        # Check if we need to stop for traffic lights
        should_stop = self.check_traffic_light(traffic_lights)
        ...
        # Accelerate back to target speed if not stopping
        if not should_stop and self.speed < self.target_speed:
            self.speed = min(self.target_speed, self.speed + 0.01)
    ...
    def check_traffic_light(self, traffic_lights):
        ...
        for light in traffic_lights:
            ...
            if self.direction in ["east", "west"] and light.direction == "horizontal":
                light_controls_me = True
            elif self.direction in ["north", "south"] and light.direction == "vertical":
                light_controls_me = True
            ...
            distance_to_light = math.sqrt((self.x - light.x)**2 + (self.z - light.z)**2)
            
            # Check if we're approaching the light and it's red or yellow
    ...
    def draw(self):
        """Draw the NPC car with different color"""
        ...
        glPushMatrix()
        glScalef(0.4, 0.2, 0.8)  # Slightly smaller than player car
        self.draw_box()
        glPopMatrix()
        
        # Add NPC car headlights (as bright spots on front of car)
        glDisable(GL_LIGHTING)  # Disable lighting for headlight glow
        glColor3f(1.0, 1.0, 0.9)  # Bright white/yellow headlights
        
        # Left headlight (smaller for NPC car)
        glPushMatrix()
        glTranslatef(-0.15, 0.08, 0.45)  # Adjusted for smaller NPC car
        glScalef(0.03, 0.03, 0.03)  # Smaller than player car headlights
        self.draw_wheel()  # Reuse wheel drawing for simple light shape
        glPopMatrix()
        
        # Right headlight (smaller for NPC car)
        glPushMatrix()
        glTranslatef(0.15, 0.08, 0.45)  # Adjusted for smaller NPC car
        glScalef(0.03, 0.03, 0.03)  # Smaller than player car headlights
        self.draw_wheel()  # Reuse wheel drawing for simple light shape
        ...
        glScalef(0.8, 0.8, 0.8)  # Smaller wheels
        self.draw_wheel()
        glPopMatrix()
