...
class Game:
    ...
    def __init__(self, width=1200, height=800, fov=45):

        self.last_displayed_time = "Night (00:00)"  # Track last displayed time for HUD updates

        pygame.init()
        self.width = width
        self.height = height
        self.fov = fov  # Field of view for perspective
        ...
        # Set up perspective - balanced view distance for performance and gameplay
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        gluPerspective(self.fov, (self.width / self.height), 0.1, 150.0)  # Adjusted far plane to 150.0 for better performance
        ...
        # Create multiple tracks for larger area 5x5
        self.tracks = []
        
        # Create center track first to get arm_length
        track_center = Track(0, 0)
        spacing = track_center.arm_length * 2  # Distance between track centers
        
        # Create 5x5 grid of tracks systematically
        # Grid positions: -2, -1, 0, 1, 2 (center at 0,0)
        for grid_x in range(-2, 3):  # -2, -1, 0, 1, 2
            for grid_z in range(-2, 3):  # -2, -1, 0, 1, 2
                x_pos = grid_x * spacing
                z_pos = grid_z * spacing
                track = Track(x_pos, z_pos)
                self.tracks.append(track)

        # NPC cars - ENABLED
        self.npc_cars = []
        self.npc_spawn_timer = 0.0
        self.npc_spawn_interval = 8.0  # Spawn a new car every 8 seconds (increased from 5 for performance)
        self.max_npc_cars = 4  # Maximum number of NPC cars (reduced from 6 for performance)
        ...
        self.camera_lag_factor = 0.15  # Increased from 0.08 for more responsive camera
        self.camera_angle_lag = 0.2    # Increased from 0.1 for more responsive angle following
        ...
        self.show_text_info = False  # Toggle text-based info (speed, gear, RPM)
        ...
        self.day_night_cycle_duration = 300.0  # 300 seconds (5 minutes) for full day/night cycle
        ...
        # Sky dome system
        self.sky = Sky()
        self.sky.set_sky_mode('night', 0.0)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if event.key == pygame.K_LEFT or event.key == pygame.K_RIGHT:
                        ...
                        print_terminal("INFO", "Vehicle Selection", f"Selected: {self.selected_vehicle.upper()}")
                    ...
                    print_terminal("INFO", "Game Reset", "Resetting game...")
                    self.reset_game()
                    ...
                    print_terminal("INFO", "Camera Lag", f"Increased - Position: {self.camera_lag_factor:.3f}, Angle: {self.camera_angle_lag:.3f}")
                    ...
                    print_terminal("INFO", "Camera Lag", f"Decreased - Position: {self.camera_lag_factor:.3f}, Angle: {self.camera_angle_lag:.3f}")
                elif event.key == pygame.K_3:
                    # Reset camera lag to default
                    self.camera_lag_factor = 0.15  # Updated to new default
                    self.camera_angle_lag = 0.2    # Updated to new default
                    print_terminal("INFO", "Camera Lag", "Reset to default")
                elif event.key == pygame.K_c:
                    # Show camera info
                    print_terminal("INFO", "Camera Lag", f"Position: {self.camera_lag_factor:.3f}, Angle: {self.camera_angle_lag:.3f}")
                    ...
                    print_terminal("INFO", "HUD", f"{'ON' if self.show_hud else 'OFF'}")
                elif event.key == pygame.K_i:
                    # Toggle text-based info display
                    self.show_text_info = not self.show_text_info
                    print_terminal("INFO", "Text Info", f"{'ON' if self.show_text_info else 'OFF'}")
    ...
    def show_help(self):
        ...
        if not self.vehicle:
            print_terminal("INFO", "Vehicle Selection", "Select your vehicle first!")
            return
            
        gear_info = self.vehicle.get_gear_info()
        print_terminal("INFO", "Game Status", f"Score: {self.score}")
        print_terminal("INFO", "Game Status", f"Vehicle: {self.vehicle.vehicle_type.upper()}")
        print_terminal("INFO", "Game Status", f"Position: ({self.vehicle.x:.1f}, {self.vehicle.z:.1f})")
        print_terminal("INFO", "Game Status", f"Speed: {self.vehicle.speed:.2f}")
        print_terminal("INFO", "Game Status", f"Current Gear: {gear_info['gear']}")
        print_terminal("INFO", "Game Status", f"RPM: {gear_info['rpm']}")
        print_terminal("INFO", "Game Status", f"Transmission: {'Auto' if gear_info['auto'] else 'Manual'}")
        print_terminal("INFO", "Game Status", "==================")
    ...
    def draw_help_menu(self):
        ...
        help_surface.fill((0, 0, 0, 180))  # Semi-transparent black background
    ...
    def show_gear_info(self):
        ...
        gear_info = self.vehicle.get_gear_info()
        print_terminal("INFO", "Gear Info", f"Gear: {gear_info['gear']}/6 | RPM: {gear_info['rpm']} | Mode: {'Auto' if gear_info['auto'] else 'Manual'} | Speed: {gear_info['speed']:.3f}")

    def show_time_info(self, hud_surface):
        """Display current time of day info on the provided HUD surface"""
        current_time = pygame.time.get_ticks() / 1000.0
        elapsed_time = current_time - self.game_start_time
        cycle_position = elapsed_time % self.day_night_cycle_duration

        # Convert to 24-hour game time (5 minute cycle = 24 hours)
        to_ingame_time = (elapsed_time % self.day_night_cycle_duration) * (24 * 3600) / self.day_night_cycle_duration
        
        hours = int(to_ingame_time // 3600) % 24
        minutes = int((to_ingame_time % 3600) // 60)

        time_of_clock = f"{hours:02}:{minutes:02}"

        # Create time display text
        # show every 10 minutes
        if minutes % 10 == 0:
            time_text = f"{self.time_of_day.replace('_', ' ').capitalize()} ({time_of_clock})"
            self.last_displayed_time = time_text
        else:
            time_text = self.last_displayed_time

        # Render the text
        try:
            time_font = pygame.font.Font("C:/Windows/Fonts/consolab.ttf", 14)
        except:
            time_font = pygame.font.SysFont("monospacebold", 14)
        
        time_surface = time_font.render(time_text, True, (255, 255, 255))
        
        # Position in top-right corner with background
        time_rect = time_surface.get_rect()
        time_rect.topright = (self.width - 10, 10)
        
        # Draw semi-transparent background
        bg_rect = time_rect.inflate(10, 6)
        pygame.draw.rect(hud_surface, (0, 0, 0, 180), bg_rect)
        
        # Draw the time text
        hud_surface.blit(time_surface, time_rect)
    ...
    def draw_hud(self):
        ...
        # Show time info on HUD
        self.show_time_info(hud_surface)
        ...
        # Draw gear grid display
        self.draw_gear_grid(hud_surface, gear_info['gear'])
    ...
    def draw_speedometer(self, surface, speed_kmh):
        ...
        needle_color = (0,150,255)  # Blue needle for night theme
    ...
    def draw_gear_grid(self, surface, current_gear):
        """Draw a visual gear grid display showing all 6 gears with current gear highlighted"""
        # Position the gear grid in the bottom-right area, above the speedometer
        grid_x = self.width - 120  # Right side, leaving space for border
        grid_y = self.height - 250  # Above the speedometer
        
        # Grid layout: 2 rows x 3 columns
        # Row 1: 1  2  3
        # Row 2: 4  5  6
        gear_size = 25  # Size of each gear box
        gear_spacing = 35  # Space between gear boxes
        
        # Draw background panel
        panel_width = 3 * gear_spacing + 10
        panel_height = 2 * gear_spacing + 10
        panel_rect = pygame.Rect(grid_x - 5, grid_y - 5, panel_width, panel_height)
        # pygame.draw.rect(surface, (30, 30, 30), panel_rect)  # Dark background
        # Border removed for transparent look
        
        # Draw title
        try:
            title_font = pygame.font.Font("C:/Windows/Fonts/consolab.ttf", 14)
        except:
            title_font = pygame.font.SysFont("monospacebold", 14)
        
        #title_surface = title_font.render("GEAR", True, (255, 255, 255))
        #title_rect = title_surface.get_rect(center=(grid_x + panel_width//2, grid_y - 15))
        #surface.blit(title_surface, title_rect)
        
        # First, calculate center points of each gear position for line connections
        gear_centers = {}
        for gear in range(1, 7):
            if gear == 1:      # Top left
                row, col = 0, 0
            elif gear == 2:    # Bottom left  
                row, col = 1, 0
            elif gear == 3:    # Top center
                row, col = 0, 1
            elif gear == 4:    # Bottom center
                row, col = 1, 1
            elif gear == 5:    # Top right
                row, col = 0, 2
            elif gear == 6:    # Bottom right
                row, col = 1, 2
            
            center_x = grid_x + col * gear_spacing + gear_size // 2
            center_y = grid_y + row * gear_spacing + gear_size // 2
            gear_centers[gear] = (center_x, center_y)
        
        # Draw H-pattern shifter lines FIRST (behind the gear boxes and numbers)
        line_color = (150, 150, 150)  # Light gray lines
        line_width = 1
        
        # Draw vertical lines connecting each pair (1-2, 3-4, 5-6)
        pygame.draw.line(surface, line_color, gear_centers[1], gear_centers[2], line_width)  # 1st-2nd
        pygame.draw.line(surface, line_color, gear_centers[3], gear_centers[4], line_width)  # 3rd-4th
        pygame.draw.line(surface, line_color, gear_centers[5], gear_centers[6], line_width)  # 5th-6th
        
        # Draw horizontal line connecting the three columns (at middle height)
        left_center_y = (gear_centers[1][1] + gear_centers[2][1]) // 2
        middle_center_y = (gear_centers[3][1] + gear_centers[4][1]) // 2
        right_center_y = (gear_centers[5][1] + gear_centers[6][1]) // 2
        
        # Horizontal line from left column to right column
        start_point = (gear_centers[1][0], left_center_y)
        end_point = (gear_centers[5][0], right_center_y)
        pygame.draw.line(surface, line_color, start_point, end_point, line_width)
        
        # Draw each gear box in classic H-pattern layout
        # Layout: 1  3  5
        #         2  4  6
        for gear in range(1, 7):  # Gears 1-6
            # Classic H-pattern positioning
            if gear == 1:      # Top left
                row, col = 0, 0
            elif gear == 2:    # Bottom left  
                row, col = 1, 0
            ...
            gear_x = grid_x + col * gear_spacing
            gear_y = grid_y + row * gear_spacing
            
            # Create gear box rectangle
            gear_rect = pygame.Rect(gear_x, gear_y, gear_size, gear_size)
            
            # Choose colors based on whether this is the current gear
            if gear == current_gear:
                # Current gear - bright highlight
                bg_color = (0, 150, 255)  # Bright blue
            ...
            # Draw gear box
            pygame.draw.rect(surface, bg_color, gear_rect)
            #pygame.draw.rect(surface, border_color, gear_rect, border_width)
            
            # Draw gear number
            try:
                gear_font = pygame.font.Font("C:/Windows/Fonts/consolab.ttf", 16)
            except:
                gear_font = pygame.font.SysFont("monospacebold", 16)
            
            gear_text = str(gear)
            text_surface = gear_font.render(gear_text, True, text_color)
            text_rect = text_surface.get_rect(center=gear_rect.center)
            surface.blit(text_surface, text_rect)
    ...
    def spawn_initial_npc_cars(self):
        """Spawn initial NPC cars on the right side of the roads"""
        spawn_positions = [
            # Horizontal roads (east-west) - right side traffic
            (-20, -1, "east"),   # East-bound cars drive in south lane (negative Z)
            (20, 1, "west"),     # West-bound cars drive in north lane (positive Z)
            
            # Vertical roads (north-south) - right side traffic  
            (-1, -20, "north"),  # North-bound cars drive in west lane (negative X)
            (1, 20, "south"),    # South-bound cars drive in east lane (positive X)
        ]
        
        for x, z, direction in spawn_positions[:2]:  # Start with only 2 cars for performance
            npc_car = NPCCar(x, z, direction, random.uniform(0.15, 0.25))
            self.npc_cars.append(npc_car)
    
    def spawn_npc_car(self):
        """Spawn a new NPC car at a random edge of the road network, driving on the right side"""
        if len(self.npc_cars) >= self.max_npc_cars:
            return
            
        # Spawn positions at edges of road network - ensuring right-side driving
        spawn_options = [
            # From east edge - cars going west drive in north lane (positive Z)
            (35, 1, "west"),
            # From west edge - cars going east drive in south lane (negative Z)
            (-35, -1, "east"),
            # From north edge - cars going south drive in east lane (positive X)
            (1, 35, "south"),
            # From south edge - cars going north drive in west lane (negative X)
            (-1, -35, "north")
        ]
    ...
    def update_npc_cars(self, dt):
        ...
        # Update each NPC car with day/night information and other cars for collision
        cars_to_remove = []
        for i, npc_car in enumerate(self.npc_cars):
            # Create list of other NPC cars (excluding current car)
            other_cars = [car for j, car in enumerate(self.npc_cars) if j != i]
            if not npc_car.update_ai(all_traffic_lights, self.is_day_time(), other_cars):
    ...
    def update_day_night_cycle(self, current_time):
        ...
        if cycle_progress < 0.2:
            ...
            self.sky.set_sky_mode('sunrise', t)
            ...
            self.sky.set_sky_mode('sunset', t)
            ...
            self.sky.set_sky_mode('night', t)
        
        # Print time changes
        if new_time_of_day != self.time_of_day:
            self.time_of_day = new_time_of_day
            print_terminal("INFO", "Time Change", f"{self.time_of_day.upper()}")
    ...
    def confirm_vehicle_selection(self):
        """Start the loading screen after vehicle selection"""
        # Create the selected vehicle
        if self.selected_vehicle == "car":
            self.vehicle = Car()
            print_terminal("INFO", "Vehicle Selection", "Starting with CAR - 6-speed transmission, stable handling")
        else:
            self.vehicle = Motorcycle()
            print_terminal("INFO", "Vehicle Selection", "Starting with MOTORCYCLE - 5-speed transmission, agile handling")

        # Initialize loading screen with game context for actual asset loading
        self.loading_screen = LoadingScreen(self.width, self.height, game_context=self)
        self.is_loading = True
        self.loading_complete = False
        
        # Exit selection mode
        self.vehicle_selection = False
        print_terminal("INFO", "Loading Game", "Loading game world... (actual asset loading in progress)")
    ...
    def draw_vehicle_selection(self):
        ...
        if not hasattr(self, 'selection_instructions_shown'):
            print_terminal("INFO", "Vehicle Selection", "=== VEHICLE SELECTION ===")
            print_terminal("INFO", "Vehicle Selection", f"Use LEFT/RIGHT arrows to select, ENTER/SPACE to confirm")
            print_terminal("INFO", "Vehicle Selection", f"CAR: 6-speed, stable handling")
            print_terminal("INFO", "Vehicle Selection", f"MOTORCYCLE: 5-speed, agile handling")
            print_terminal("INFO", "Vehicle Selection", f"Current selection: {self.selected_vehicle.upper()}")
        ...
        pygame.draw.rect(selection_surface, (0, 100, 255), bike_body_rect)
    ...
    def draw_construction_barriers(self):
        """Draw construction-style barriers completely surrounding the map perimeter"""
        barrier_height = 0.5
        barrier_width = 0.3
        
        # Construction barrier color - gray
        glColor3f(0.25, 0.25, 0.25)
        
        # Barrier spacing for segmented construction barrier effect
        barrier_spacing = 2.0
        
        # Map edges: ±150 in each direction (complete perimeter)
        map_edge_north = 150.0
        map_edge_south = -150.0
        map_edge_east = 150.0
        map_edge_west = -150.0
        
        def draw_barrier_segment(x1, z1, x2, z2):
            """Draw a single barrier segment (3D block)"""
            glBegin(GL_QUADS)
            # Front face
            glVertex3f(x1, 0.01, z1)
            glVertex3f(x2, 0.01, z1)
            glVertex3f(x2, barrier_height, z1)
            glVertex3f(x1, barrier_height, z1)
            # Back face
            glVertex3f(x1, 0.01, z2)
            glVertex3f(x2, 0.01, z2)
            glVertex3f(x2, barrier_height, z2)
            glVertex3f(x1, barrier_height, z2)
            # Left face
            glVertex3f(x1, 0.01, z1)
            glVertex3f(x1, 0.01, z2)
            glVertex3f(x1, barrier_height, z2)
        ...
        # Draw complete perimeter barriers with no gaps
        # NORTH edge (Z = +150, X from -150 to +150)
        x = map_edge_west
        while x < map_edge_east:
            seg_end = min(x + barrier_spacing * 0.8, map_edge_east)
            draw_barrier_segment(x, map_edge_north - barrier_width, seg_end, map_edge_north)
            x += barrier_spacing
        
        # SOUTH edge (Z = -150, X from -150 to +150)
        x = map_edge_west
        while x < map_edge_east:
            seg_end = min(x + barrier_spacing * 0.8, map_edge_east)
            draw_barrier_segment(x, map_edge_south, seg_end, map_edge_south + barrier_width)
            x += barrier_spacing
        
        # EAST edge (X = +150, Z from -150 to +150)
        z = map_edge_south
        while z < map_edge_north:
            seg_end = min(z + barrier_spacing * 0.8, map_edge_north)
            draw_barrier_segment(map_edge_east - barrier_width, z, map_edge_east, seg_end)
            z += barrier_spacing
        
        # WEST edge (X = -150, Z from -150 to +150)
        z = map_edge_south
        while z < map_edge_north:
            seg_end = min(z + barrier_spacing * 0.8, map_edge_north)
            draw_barrier_segment(map_edge_west, z, map_edge_west + barrier_width, seg_end)
            z += barrier_spacing

    def draw_minimap(self):
        """Draw a rotating mini-map in the bottom-left corner showing the complete 5x5 track grid"""
        # Only draw minimap if vehicle is initialized
        if not self.vehicle:
            return
            
        # Mini-map properties
        map_size = 150  # Size of the mini-map in pixels
        map_x = 20      # X position from left edge
        map_y = self.height - 20  # Y position from bottom
        
        # Get player position and angle
        player_x = self.vehicle.x
        player_z = self.vehicle.z
        player_angle = self.vehicle.angle  # Use angle directly from vehicle (in degrees)
        
        # Create a larger surface for the complete world map (we'll crop it later)
        world_map_size = map_size * 3  # Even larger to avoid clipping when rotating
        world_map_surface = pygame.Surface((world_map_size, world_map_size))
        world_map_surface.fill((20, 20, 20))  # Dark background
        
        # Calculate scale factor based on actual game world size
        # Total world size: 5x5 grid of tracks, each track is 60x60 units
        # So total world is 300x300 units (from -150 to +150)
        world_total_size = 300.0  # Total world spans 300 units
        scale = world_map_size / world_total_size
        world_center = world_map_size // 2
        
        # Track dimensions from the actual track class
        track_arm_length = 30.0  # Each arm extends 30 units from center
        track_width = 4.0
        half_width = track_width / 2
        track_spacing = track_arm_length * 2  # 60 units between track centers
        
        # Draw the complete 5x5 grid of tracks (static, non-rotated)
        # Grid positions: -2, -1, 0, 1, 2 (matching the actual game)
        for grid_x in range(-2, 3):  # -2, -1, 0, 1, 2
            for grid_z in range(-2, 3):  # -2, -1, 0, 1, 2
                # Calculate track center position in world coordinates
                track_center_x = grid_x * track_spacing  # -120, -60, 0, 60, 120
                track_center_z = grid_z * track_spacing  # -120, -60, 0, 60, 120
                
                # Convert to map coordinates (standard screen coordinates)
                map_track_x = world_center + int(track_center_x * scale)
                map_track_z = world_center - int(track_center_z * scale)  # Negative for screen Y-axis
                
                # Draw horizontal road
                h_road_left = map_track_x - int(track_arm_length * scale)
                h_road_right = map_track_x + int(track_arm_length * scale)
                h_road_top = map_track_z - int(half_width * scale)
                h_road_bottom = map_track_z + int(half_width * scale)
                
                h_road_rect = pygame.Rect(h_road_left, h_road_top, 
                                        h_road_right - h_road_left, 
                                        h_road_bottom - h_road_top)
                pygame.draw.rect(world_map_surface, (80, 80, 80), h_road_rect)
                
                # Draw vertical road
                v_road_left = map_track_x - int(half_width * scale)
                v_road_right = map_track_x + int(half_width * scale)
                v_road_top = map_track_z - int(track_arm_length * scale)
                v_road_bottom = map_track_z + int(track_arm_length * scale)
                
                v_road_rect = pygame.Rect(v_road_left, v_road_top,
                                        v_road_right - v_road_left,
                                        v_road_bottom - v_road_top)
                pygame.draw.rect(world_map_surface, (80, 80, 80), v_road_rect)
        
        # Create the final mini-map surface
        minimap_surface = pygame.Surface((map_size, map_size))
        minimap_surface.fill((20, 20, 20))  # Dark background
        
        # Calculate player position on the original world map (before rotation)
        player_map_x = world_center + int(player_x * scale)
        player_map_z = world_center - int(player_z * scale)  # Negative for screen Y-axis
        
        # To make the player the true rotation axis, we sample the world map around the player
        # using inverse rotation for each pixel in the mini-map
        half_map = map_size // 2
        
        # Use the exact same angle system as the game movement:
        # Game movement: new_x = x + sin(angle) * speed, new_z = z + cos(angle) * speed
        # This means angle 0° = North, 90° = East, 180° = South, 270° = West
        # For the mini-map, we want the player's forward direction to point "up"
        angle_rad = math.radians(player_angle)  # Remove 180 degrees to flip map back
        
        # Create rotation matrix that rotates the world opposite to player direction
        # So when player faces East (90°), the world rotates -90° making East point up
        cos_angle = math.cos(-angle_rad)
        sin_angle = math.sin(-angle_rad)
        
        for mini_y in range(map_size):
            for mini_x in range(map_size):
                # Calculate position relative to mini-map center (player position)
                rel_x = mini_x - half_map
                rel_y = (map_size - 1 - mini_y) - half_map  # Flip Y-axis: top becomes bottom
                
                # Apply rotation to match game's coordinate system
                # Game uses: x + sin(angle), z + cos(angle)
                # So we map: mini_x -> world_x direction, mini_y -> world_z direction
                world_rel_x = -rel_x * cos_angle - rel_y * sin_angle
                world_rel_z = -rel_x * sin_angle + rel_y * cos_angle
                
                # Calculate actual world map coordinates
                world_x = int(player_map_x + world_rel_x)
                world_y = int(player_map_z - world_rel_z)  # Note: minus because screen Y is flipped
                
                # Check if the source position is within the world map bounds
                if (0 <= world_x < world_map_size and 0 <= world_y < world_map_size):
                    # Copy pixel from world map to mini-map
                    pixel_color = world_map_surface.get_at((world_x, world_y))
                    minimap_surface.set_at((mini_x, mini_y), pixel_color)
        
        # Draw player as yellow triangle pointing up (always points up since map rotates around it)
        triangle_size = 12
        center = map_size // 2
        triangle_points = [
            (center, center - triangle_size),      # Top point
            (center - triangle_size//2, center + triangle_size//2),  # Bottom left
            (center + triangle_size//2, center + triangle_size//2),  # Bottom right
        ]
        pygame.draw.polygon(minimap_surface, (0, 150, 255), triangle_points)  # Cyan triangle

        # light gray border around pointer / player triangle
        # pygame.draw.polygon(minimap_surface, (200, 200, 200), triangle_points, 1)

        
        # Draw border around mini-map
        pygame.draw.rect(minimap_surface, (100, 100, 100), (0, 0, map_size, map_size), 2)
        
        # Draw border around mini-map
        pygame.draw.rect(minimap_surface, (100, 100, 100), (0, 0, map_size, map_size), 2)
        
        # Convert surface to OpenGL texture and display
        glMatrixMode(GL_PROJECTION)
        glPushMatrix()
        glLoadIdentity()
        glOrtho(0, self.width, self.height, 0, -1, 1)  # 2D orthographic projection
        ...
        glRasterPos2f(map_x, map_y)
        glDrawPixels(map_size, map_size, GL_RGBA, GL_UNSIGNED_BYTE, minimap_data)
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.vehicle_selection:
            ...
            elif self.is_loading and not self.loading_complete:
                # Loading screen phase
                self.loading_screen.update(dt)
                
                # Clear screen for loading display
                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
                ...
                if self.loading_screen.is_complete():
                    ...
                    # Set initial position and angle for vehicle
                    self.vehicle.x = 10.0
                    self.vehicle.angle = 270.0
                    
                    # Auto-enable headlights at night
                    if not self.is_day_time():
                        self.vehicle.headlights_on = True

                    print_terminal("INFO", "Game Started", "Game started! Use arrow keys to drive, L for headlights, T for transmission mode")
                    print_terminal("INFO", "Auto Headlights", "Auto headlights ON (nighttime)")
            else:
                ...
                # Check for NPC car collisions
                #if self.vehicle.check_npc_collision(self.npc_cars):
                #    self.vehicle.resolve_collision(self.vehicle.prev_x, self.vehicle.prev_z)
                
                # Check for street light collisions
                for track in self.tracks:
                    for street_light in track.street_lights:
                        if street_light.check_collision(self.vehicle.x, self.vehicle.z):
                            self.vehicle.resolve_collision(self.vehicle.prev_x, self.vehicle.prev_z)
                            break
                
                # Check for traffic light collisions
                for track in self.tracks:
                    for traffic_light in track.traffic_lights:
                        if traffic_light.check_collision(self.vehicle.x, self.vehicle.z):
                            self.vehicle.resolve_collision(self.vehicle.prev_x, self.vehicle.prev_z)
                            break
                ...
                for track in self.tracks:
                    for sign in track.signs:
                        if sign.check_collision(self.vehicle.x, self.vehicle.z):
                            self.vehicle.resolve_collision(self.vehicle.prev_x, self.vehicle.prev_z)
                            break
                
                # Check for curb/sidewalk edge collisions (blocks entry from road)
                if self.vehicle.check_sidewalk_collision(self.tracks):
                    self.vehicle.resolve_collision(self.vehicle.prev_x, self.vehicle.prev_z)
                
                # Check for barrier collisions at map edges
                if self.vehicle.check_barrier_collision():
                    self.vehicle.resolve_collision(self.vehicle.prev_x, self.vehicle.prev_z)
                
                # Track if car is currently on sidewalk surface
                self.vehicle.on_sidewalk = self.vehicle.is_on_sidewalk(self.tracks)
                ...
                # Draw sky dome first (before everything else)
                self.sky.draw()
                
                # Draw everything (with distance culling for performance)
                for track in self.tracks:
                    track.draw(self.is_day_time(), self.camera_x, self.camera_z)  # Pass camera position for culling
                self.vehicle.draw()
                
                # Draw construction barriers at map edges
                self.draw_construction_barriers()
        ...
        # Cleanup
        print_terminal("INFO", "Game Exit", "Exiting game...")
        pygame.quit()
        sys.exit()