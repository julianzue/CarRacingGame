...
class WindowSizeSelector:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    # Cycle left through sizes
                    sizes = ["small", "normal", "portrait"]
                    current_index = sizes.index(self.selected_size)
                    self.selected_size = sizes[(current_index - 1) % len(sizes)]
                    print_terminal("INFO", "Size Selection", f"Selected: {self.selected_size.upper()} size")
                elif event.key == pygame.K_RIGHT:
                    # Cycle right through sizes
                    sizes = ["small", "normal", "portrait"]
                    current_index = sizes.index(self.selected_size)
                    self.selected_size = sizes[(current_index + 1) % len(sizes)]
                    print_terminal("INFO", "Size Selection", f"Selected: {self.selected_size.upper()} size")
    ...
    def draw(self):
        ...
        box_width = 140
        box_height = 120
        small_x = self.width//2 - 160
        normal_x = self.width//2
        portrait_x = self.width//2 + 160
        box_y = self.height//2
        ...
        # Portrait size box
        portrait_color = (0, 100, 0) if self.selected_size == "portrait" else (40, 40, 40)
        portrait_border_color = (0, 200, 0) if self.selected_size == "portrait" else (100, 100, 100)
        
        portrait_rect = pygame.Rect(portrait_x - box_width//2, box_y - box_height//2, box_width, box_height)
        pygame.draw.rect(self.screen, portrait_color, portrait_rect)
        pygame.draw.rect(self.screen, portrait_border_color, portrait_rect, 3 if self.selected_size == "portrait" else 1)
        
        # Portrait size label and info
        portrait_label = "PORTRAIT"
        portrait_label_surface = self.font_medium.render(portrait_label, True, (255, 255, 255))
        portrait_label_rect = portrait_label_surface.get_rect(center=(portrait_x, box_y - 35))
        self.screen.blit(portrait_label_surface, portrait_label_rect)
        
        portrait_info = ["1920 x 1080", "50° view", "Portrait"]
        for i, info in enumerate(portrait_info):
            info_surface = self.font_small.render(info, True, (200, 200, 200))
            info_rect = info_surface.get_rect(center=(portrait_x, box_y + i * 15))
            self.screen.blit(info_surface, info_rect)
        
        # Selection indicator
        if self.selected_size == "small":
            # right side polygon points on right side of small box no gap with box
            arrow_points = [
                (small_x + box_width//2 + 15, box_y),
                (small_x + box_width//2 + 5, box_y - 10),
                (small_x + box_width//2 + 5, box_y + 10)
            ]
            pygame.draw.polygon(self.screen, (255, 255, 0), arrow_points)
        elif self.selected_size == "normal":
            arrow_points = [
                (normal_x - box_width//2 - 15, box_y),
                (normal_x - box_width//2 - 5, box_y - 10),
                (normal_x - box_width//2 - 5, box_y + 10)
            ]
            pygame.draw.polygon(self.screen, (255, 255, 0), arrow_points)

            # right polygon points on right
            arrow_points = [
                (normal_x + box_width//2 + 15, box_y),
                (normal_x + box_width//2 + 5, box_y - 10),
                (normal_x + box_width//2 + 5, box_y + 10)
            ...
            # left polygon points on left
            arrow_points = [
                (portrait_x - box_width//2 - 15, box_y),
                (portrait_x - box_width//2 - 5, box_y - 10),
                (portrait_x - box_width//2 - 5, box_y + 10)
            ]
            pygame.draw.polygon(self.screen, (255, 255, 0), arrow_points)
    ...
    def get_size_info(self):
        ...
        if self.selected_size == "small":
            ...
            return "1920x1080, 50° view"
    
    def run(self):
        """Run the size selection loop"""
        print_terminal("INFO", "Window Size Selection", "SMALL: 800x600 with 30° perspective")
        print_terminal("INFO", "Window Size Selection", "NORMAL: 1200x800 with 45° perspective")
        print_terminal("INFO", "Window Size Selection", "PORTRAIT: 1920x1080 with 50° perspective")
        print_terminal("INFO", "Window Size Selection", "Use LEFT/RIGHT arrows to select, ENTER/SPACE to confirm")
...
def main():
    try:
        ...
        print_terminal("INFO", "Window Size Selection", f"Selected window size: {selected_size.upper()}")
        ...
        if selected_size == "small":
            ...
            game = Game(width=1920, height=1080, fov=50)

        print_terminal("INFO", "Game Initialization", "Game initialized successfully!")
        print_terminal("INFO", "Game Start", "Starting night racing game...")
        game.run()
