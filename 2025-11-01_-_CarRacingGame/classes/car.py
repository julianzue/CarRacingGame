...
class Car(Vehicle):
    ...
    def draw(self):
        ...
        # Car body (a simple box) - Resized to be proportional to 4.0 unit wide street
        glColor3f(0.0, 0.0, 1.0)  # Blue car
        glBegin(GL_QUADS)
        
        # Front face - Car is now 1.0 units wide (±0.5) and 2.4 units long (±1.2)
        glVertex3f(-0.5, 0.0, 1.2)
        glVertex3f(0.5, 0.0, 1.2)
        glVertex3f(0.5, 0.6, 1.2)
        glVertex3f(-0.5, 0.6, 1.2)
        ...
        # Back left wheel (adjusted for narrower car)
        glPushMatrix()
        glTranslatef(-0.45, -0.1, -0.8)  # Adjusted position for narrower car
        self.draw_wheel()
        glPopMatrix()
        
        # Back right wheel (adjusted for narrower car)
        glPushMatrix()
        glTranslatef(0.45, -0.1, -0.8)  # Adjusted position for narrower car
        self.draw_wheel()
        glPopMatrix()
...
class Motorcycle(Vehicle):
    ...
    def draw(self):
        ...
        # Add motorcycle headlight (single centered headlight)
        if self.headlights_on:
            glDisable(GL_LIGHTING)
